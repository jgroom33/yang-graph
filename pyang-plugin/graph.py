...
class YangGraphSingle(plugin.PyangPlugin):
    ...
    def add_opts(self, optparser):
        optlist = [
            optparse.make_option(
                "--config-only", action="store_true", dest="config_only",
    ...
    def emit(self, ctx, modules, fd):
        logging.debug(f"Entering emit with {len(modules)} modules: {[m.arg for m in modules]}")
        if not modules:
        ...
        try:
            json_data = json.dumps(result, indent=2, ensure_ascii=False, sort_keys=False)
            fd.write(json_data)
    ...
    def _build_graph(self, stmt, result, config_only, module=None):
        ...
        if config_only and stmt.keyword in ("container", "leaf", "leaf-list", "list") and not stmt.i_config:
            return

        if stmt.keyword == "list":
            list_name = qualify_name(stmt, module)
            description = stmt.search_one("description")
            description_str = preprocess_string(description.arg) if description else "No description available"
            ...
            for child in stmt.i_children:
                if child.keyword == "leaf":
                    ...
                    if type_stmt and type_stmt.arg == "leafref":
                        ...
                        if path_stmt:
                            logging.debug(f"Found leafref in {list_name}: {path_stmt.arg}")
                            try:
                                target_module, target_list, target_key = self._resolve_leafref_path(path_stmt.arg, module)
                                if target_list and target_key:
                                    ...
                                    if list_name != target_full_name:
                                        ...
                                        logging.debug(f"Added potential edge: {list_name} -> {target_full_name}")
                                    else:
                                        logging.debug(f"Skipped self-reference: {list_name}")
                                else:
                                    logging.debug(f"Leafref {path_stmt.arg} resolved to no valid target")
                            except Exception as e:
                                logging.warning(f"Failed to resolve leafref {path_stmt.arg}: {e}")
    ...
    def _resolve_leafref_path(self, path, source_module):
        ...
        for i, part in enumerate(parts):
            if ":" in part:
                ...
                target_module = next(
                    (m for m in source_module.i_ctx.modules.values() if m.search_one("prefix") and m.search_one("prefix").arg == prefix),
                    source_module
                )
        ...
        if target_module and target_list and target_key:
            logging.debug(f"Potential target: {target_module.arg}:{target_list} with key {target_key}")
            return target_module, target_list, target_key
        logging.debug(f"Path resolution incomplete: module={target_module.arg if target_module else None}, list={target_list}, key={target_key}")
